    ...
    return (
        <div className="nav-container" data-testid="navbar">
            <nav className="px-5 w-full h-10 bg-pedals-yellow flex flex-row justify-between items-center font-nunito bg-deep-yellow">
                <Link 
                    href="/recipe-book" 
                    className={`text-deep-brown hover:text-gray-700 transition duration-300 text-pedals-brown ${currentPage === '/recipe-book' ? 'bg-white':'bg-transparent'}`}
                ...
                <Link 
                    href="/" 
                    className={`text-deep-brown hover:text-gray-700 font-bold text-xl transition duration-300 text-pedals-brown ${currentPage === '/' ? 'bg-white':'bg-transparent'}`}
                ...
                <div className="flex items-center">
                    {user && (
                        <Link
                            ...
                            className={`text-deep-brown hover:text-gray-700 transition duration-300 text-pedals-brown ${currentPage === '/profile' ? 'bg-white':'bg-transparent'}`}
                            onClick={() => handlePageChange('/profile')} // Set current page on click
                        >
                            <div className="py-2 px-4 rounded">My Profile</div> {/* Added padding and rounded corners */}
                        </Link>
                    )}
                </div>
            </nav>
